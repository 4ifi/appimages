...
git clone -b "public" https://git.magicalgirl.moe/james/Kart-Public.git
cd Kart-Public
mkdir tmp
mkdir assets/installer
cd tmp
curl --retry 9999 --retry-delay 3 --speed-time 10 --retry-max-time 0 -C - -L https://github.com/STJr/Kart-Public/releases/download/v1.1/srb2kart-v11-x64-Installer.exe -o installer.exe
curl --retry 9999 --retry-delay 3 --speed-time 10 --retry-max-time 0 -C - -L https://github.com/STJr/Kart-Public/releases/download/v1.1/srb2kart-v11-x64-Patch.zip -o patch.zip
7z x installer.exe
7z x -y patch.zip
...
echo -e 'SRB2WADDIR=$HERE/usr/bin LD_LIBRARY_PATH=$HERE/usr/lib:$LD_LIBRARY_PATH exec $HERE/usr/bin/srb2kart-* -opengl "$@"' >> AppDir/AppRun
